<?php
// rodzina_i_zdrowie CPT
if(!function_exists('custom_post_type_rodzina_i_zdrowie')) {
    function custom_post_type_rodzina_i_zdrowie() {
        $post_type_name = 'rodzina-i-zdrowie';
        $taxonomy_name = 'kategorie-' . $post_type_name;

        $labels = array(
            'name'                => _x( 'Rodzina i Zdrowie', 'Post Type General Name', 'sputnik-wp-theme' ),
            'singular_name'       => _x( 'Rodzina i Zdrowie', 'Post Type Singular Name', 'sputnik-wp-theme' ),
            'menu_name'           => __( 'Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'parent_item_colon'   => __( 'Nadrzędna Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'all_items'           => __( 'Wszystkie Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'view_item'           => __( 'Zobacz Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'add_new_item'        => __( 'Dodaj nową Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'add_new'             => __( 'Dodaj nową', 'sputnik-wp-theme' ),
            'edit_item'           => __( 'Edytuj Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'update_item'         => __( 'Zaaktualizuj Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'search_items'        => __( 'Wyszukaj Rodzina i Zdrowie', 'sputnik-wp-theme' ),
        ...
        $args = array(
            'label'               => __( 'Rodzina i Zdrowie', 'sputnik-wp-theme' ),
            'description'         => '',
            'labels'              => $labels,
    ...
    add_action( 'init', 'custom_post_type_rodzina_i_zdrowie', 0 );
}
...
if(!function_exists('tax_custom_post_type_rodzina_i_zdrowie_categories')) {
    ...
    add_action( 'init', 'tax_custom_post_type_rodzina_i_zdrowie_categories', 0 );
}