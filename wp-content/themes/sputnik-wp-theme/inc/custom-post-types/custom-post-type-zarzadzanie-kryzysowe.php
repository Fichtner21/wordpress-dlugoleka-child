<?php
// kryzysowe CPT
if(!function_exists('custom_post_type_kryzysowe')) {
    function custom_post_type_kryzysowe() {
        $post_type_name = 'kryzysowe';
        $taxonomy_name = 'kategorie-' . $post_type_name;
        $post_type_rewrite = $post_type_name . '-' . 'lista';

        $labels = array(
            'name'                => _x( 'Zarządzanie Kryzysowe', 'Post Type General Name', 'sputnik-wp-theme' ),
            'singular_name'       => _x( 'Zarządzanie Kryzysowe', 'Post Type Singular Name', 'sputnik-wp-theme' ),
            'menu_name'           => __( 'Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'parent_item_colon'   => __( 'Nadrzędna Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'all_items'           => __( 'Wszystkie Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'view_item'           => __( 'Zobacz Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'add_new_item'        => __( 'Dodaj nową Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'add_new'             => __( 'Dodaj nową', 'sputnik-wp-theme' ),
            'edit_item'           => __( 'Edytuj Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'update_item'         => __( 'Zaaktualizuj Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'search_items'        => __( 'Wyszukaj Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
        ...
        $args = array(
            'label'               => __( 'Zarządzanie Kryzysowe', 'sputnik-wp-theme' ),
            'description'         => '',
            'labels'              => $labels,
    ...
    add_action( 'init', 'custom_post_type_kryzysowe', 0 );
}
...
if(!function_exists('tax_custom_post_type_kryzysowe_categories')) {
    ...
    add_action( 'init', 'tax_custom_post_type_kryzysowe_categories', 0 );
}